...
main:
	...
	sprintf(resFileName, "E:/TLR_DATA/KR_D/results/KR_D_RESULT_TRACKED.csv");
	...
	for (int frameIdx = stx; frameIdx <= etx; frameIdx++) 
	{
		...
		//Color segmentation!!
		colorSegmentation(src, redMap, greenMap, yellowMap, ryMap);			// gpu segmentation.    IMPORTANT!!!!
		
		//vector<CvRect> redBlobs = blobDetection(ryMap);
		vector<CvRect> redBlobs = blobDetection(redMap);
		...
		//cvShowImage("redBlobs", redMap);
		//cvShowImage("greenBlobs", greenMap);

		// Step :: RED 블롭에 대한 인식 및 검증
		for (int i = 0; i < redBlobs.size(); i++) {
			if (!checkAspectRatio(redBlobs.at(i))) continue;
			if( !checkBulbValidation( res, redBlobs.at(i), 1 ) ) continue;		// Rectangular verification

			// 네모상자 그리는 부분
			CvRect redBlob = redBlobs.at(i);		// 블롭 한 개씩 꺼내서 담기
			CvPoint p1, p2;
			p1.x = (redBlob.x / scaleFactor) - (redBlob.width / scaleFactor) * 0.5;
			p2.x = (redBlob.x / scaleFactor) + (redBlob.width / scaleFactor) * 1.5;
			p1.y = (redBlob.y / scaleFactor) - (redBlob.height / scaleFactor) * 0.5;
			p2.y = (redBlob.y / scaleFactor) + (redBlob.height / scaleFactor) * 1.5;
			cvDrawRect(res, p1, p2, CV_RGB(255, 0, 0), 2);

			IplImage *roiImg = getBulbRoiImage(src, redBlobs.at(i), 1);			// 신호등 박스 추출해서 가져옴
		...
		for (int i = 0; i < greenBlobs.size(); i++) {
			if (!checkAspectRatio(greenBlobs.at(i))) continue;						// bulb가 정사각형에 가까운지를 보고 인정할지 결정
			if( !checkBulbValidation( res, greenBlobs.at(i), 2 ) ) continue;		// bulb옆 rectangular verification

			// 네모상자 그리는 부분
			CvRect greenBlob = greenBlobs.at(i);		// 블롭 한 개씩 꺼내서 담기
			CvPoint p1, p2;

			p1.x = (greenBlob.x / scaleFactor) - (greenBlob.width / scaleFactor) * 0.5;
			p2.x = (greenBlob.x / scaleFactor) + (greenBlob.width / scaleFactor) * 1.5;
			p1.y = (greenBlob.y / scaleFactor) - (greenBlob.height / scaleFactor) * 0.5;
			p2.y = (greenBlob.y / scaleFactor) + (greenBlob.height / scaleFactor) * 1.5;
			cvDrawRect(res, p1, p2, CV_RGB(0, 255, 255), 2);
...
{
	char buf[500];
	sprintf_s(buf, "E:/TLR_DATA/KR_D/results/%.5d.jpg", fIdx);
	...
	sprintf_s(buf, "E:/TLR_DATA/KR_D/%.5d.jpg", fIdx);
...
IplImage* getBulbRoiImage(IplImage *src, CvRect rt, int mode)
{
	...
	if (mode == 1) {
		// Case :: Red
		x = rt.x - rt.width * marginRate;		// x - width*
		y = rt.y - rt.height * marginRate;
		width = rt.width + rt.width * marginRate * 2;
		height = rt.height * 3 + rt.height * marginRate * 2;
	}
...
void saveBulbRoiImage(IplImage *src, CvRect rt, int frameSeq, int mode)
{
	...
	char buf[100];
	if (mode == 1)
		sprintf_s(buf, "E:/TLR_DATA/KR_D/results/red/%.5d.jpg", frameSeq, roiIdx++);
	else if (mode == 2)
		sprintf_s(buf, "E:/TLR_DATA/KR_D/results/green/%d_%d.jpg", frameSeq, roiIdx++);
	else if (mode == 3)
		sprintf_s(buf, "E:/TLR_DATA/KR_D/results/%d_%d.jpg", frameSeq, roiIdx++);
	else if (mode == 4)
		sprintf_s(buf, "E:/TLR_DATA/KR_D/results/green-am/%d_%d.jpg", frameSeq, roiIdx++);
	else if (mode == 5)
		sprintf_s(buf, "E:/TLR_DATA/KR_D/results/yellow/%d_%d.jpg", frameSeq, roiIdx++);
	else if (mode == 6)
		sprintf_s(buf, "E:/TLR_DATA/KR_D/results/etc/%d_%d.jpg", frameSeq, roiIdx++);
...
bool checkBulbValidation(IplImage *src, CvRect rt, int mode)
{
	...
	if (mode == 1) {
		// Case :: Red
		x = rt.x / scaleFactor + rt.width / scaleFactor * 1.1;
		y = rt.y / scaleFactor;
		width = rt.width / scaleFactor * 2;
		height = rt.height / scaleFactor;
	}
	else if (mode == 2) {
		// Case :: Green
		x = rt.x / scaleFactor - rt.width / scaleFactor * 2;
		y = rt.y / scaleFactor;
		width = rt.width / scaleFactor*1.9;
		height = rt.height / scaleFactor;
	}
	if (y < 0) y = 0;
	...
	if (mode == 1 && avgVal <= 120)		// red light case
	{
		//? verification 네모상자 그리는 부분
		CvPoint p1, p2;
		p1.x = x;
		p2.x = x + width;
		p1.y = y;
		p2.y = y + height;
		cvDrawRect(src, p1, p2, CV_RGB(0, 0, 0), 2);
		//?
		printf(" Mode=%d, (%d, %d, %d, %d) = %d\n", mode, rt.x, rt.y, width, height, avgVal);
		return true;
	}
	if (mode == 2 && avgVal <= 120)		// green light case
	{
		//? verification 네모상자 그리는 부분
		CvPoint p1, p2;
		p1.x = x;
...
bool checkAspectRatio(CvRect rt)			// 정사각형에 근접하지 않은 것은 버리기 위함.
{
	if (rt.width < 7 || rt.height < 7) return false;		// 너무 작은것은 인정안함
	if (rt.width > rt.height) {
		if (abs((float)rt.height / (float)rt.width) <= 0.6) return false;	// 가로가 세로보다 크면서 and (높이/너비)^2가 0.6보다 작으면 인정안함
	}
	else {
		if (abs((float)rt.width / (float)rt.height) <= 0.6) return false;	// 세로가 가로보다 더 크면서 and (너비/높이)^2가 0.6보다 작으면 인정안함
	}
	return true;
}
...
void loadResult()
{
	char fName[200];
	fstream inputFile;
	sprintf_s(fName, "E:/TLR_DATA/KR_D/results/KR_D_RESULT_TRACKED.csv");
...
void loadGT()		// 실제 dataset의 Ground truth 읽기
{
	char fName[200];
	fstream inputFile;
	sprintf_s(fName, "E:/TLR_DATA/KR_D/results/KR_D_GT_revised.csv");		// GT 파일
	inputFile.open(fName, ios::in);
	int data[6];
	if (inputFile.is_open()) {
		string line = "";
